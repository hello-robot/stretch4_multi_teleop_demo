...
class MediaPipeBaseNode(InputInterfaceNode):
    def __init__(self, node_name, initial_axis_names=None, initial_button_names=None, **kwargs):
        # 6DOF base axes are always present: x, y, z, roll, pitch, yaw
        base_axes = ['x', 'y', 'z', 'roll', 'pitch', 'yaw']
        if initial_axis_names:
            base_axes.extend(initial_axis_names)
        
        super().__init__(node_name, base_axes, initial_button_names or [], **kwargs)
        ...
        self.declare_parameter('tracked_points', [], descriptor=ParameterDescriptor(read_only=True))
    ...
    def _load_config(self, path):
        try:
            with open(path, 'r') as f:
                ...
                if 'virtual_axes' in config:
                    for axis in config['virtual_axes']:
    ...
    def add_virtual_item(self, item):
        ...
        if item['type'] == 'axis':
            if name not in self._axes:
                ...
                self._last_axes.append(-1.0) # Initial value for distance-based axis
        ...
        # Note: We don't dynamically update the Joy message structure easily in this base class 
        # without potentially confusing subscribers, but InputInterfaceNode's publish_input 
        # will reflect the new lengths.
    ...
    def set_tracked_points(self, names):
        self._tracked_point_names = names
        # We can't update read_only params easily after declaration, 
        # but we can declare it with the list if we know it early.
